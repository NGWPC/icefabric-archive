...
def test_module_ipes():
    ...
    # LASAM
    lasam_pydantic_models0 = get_lasam_parameters(
        catalog, domain, identifier, sft_included=False, soil_params_file="vG_default_params_HYDRUS.dat"
    )
    lasam_pydantic_models1 = get_lasam_parameters(
        catalog, domain, identifier, sft_included=True, soil_params_file="vG_default_params_HYDRUS.dat"
    )

    # Snow17
    snow17_pydantic_models0 = get_snow17_parameters(catalog, domain, identifier, conus_only=False, envca=True)
    snow17_pydantic_models1 = get_snow17_parameters(catalog, domain, identifier, conus_only=True, envca=False)

    # SAC SMA
    sacsma_pydantic_models0 = get_sacsma_parameters(catalog, domain, identifier, conus_only=True, envca=False)
    sacsma_pydantic_models1 = get_sacsma_parameters(catalog, domain, identifier, conus_only=True, envca=True)

    # SMP
    smp_pydantic_models = get_smp_parameters(catalog, domain, identifier, "TopModel")
    ...
    for i in range(1):
        ...
        assert len(snow17_variables0) == 63, (
            "Incorrect Number of Attributes For Snow17 (w/ conus_only=False, envca=True)"
        )

        snow17_variables1 = [attr for attr in dir(snow17_pydantic_models1[i]) if not attr.startswith("__")]
        assert len(snow17_variables1) == 63, (
            "Incorrect Number of Attributes For Snow17 (w/ conus_only=True, envca=False)"
        )

        sacsma_variables0 = [attr for attr in dir(sacsma_pydantic_models0[i]) if not attr.startswith("__")]
        assert len(sacsma_variables0) == 55, (
            "Incorrect Number of Attributes For SACSMA (w/ conus_only=True, envca=False)"
        )

        sacsma_variables1 = [attr for attr in dir(sacsma_pydantic_models1[i]) if not attr.startswith("__")]
        assert len(sacsma_variables1) == 55, (
            "Incorrect Number of Attributes For SACSMA (w/ conus_only=True, envca=True)"
        )
