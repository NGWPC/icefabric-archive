...
import icefabric_tools.icechunk.nc_conv_utils as ncc_utils
from icefabric_tools.icechunk.icechunk_s3_module import (
    IcechunkS3Repo,
    NGWPCLocations,
    S3Path,
)
...
def snodas_virtualize_push_test(
    ...
    """
    snodas_repo = IcechunkS3Repo(
        location=new_ic_repo, virtual_chunk_mapping=[{"bucket": "ngwpc-forcing", "region": "us-east-1"}]
    )
    nc_virt_ds = awf_utils.virtualize_and_concat_archival_files_on_time(
...
def get_nc_by_year(years: range):
    ...
    for y in years:
        print(y)
        files += awf_utils.get_archival_weather_files(
