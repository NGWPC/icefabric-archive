...
async def get_xs_subset_gpkg(identifier: str = Path(...,
                                                    description="HUC-8 identifier to filter by huc ID",
                                                    example=["02040106"]),
                             xstype: XsType = Query(XsType.MIP, 
                                                    description="The iceberg namespace used to query the cross-sections"
    )
):
    """
    ...
    try:
        # Create data subset
        data_gdf=subset_xs(catalog=catalog, 
        ...
        if not tmp_path.exists():
            raise HTTPException(status_code=500,
                                detail=f"Failed to create geopackage file at {tmp_path}.")
        if tmp_path.stat().st_size == 0:
            tmp_path.unlink(missing_ok=True)
            raise HTTPException(status_code=404, 
                                detail=f"No data found for HUC {identifier}.")

        # Verify it's actually a file, not a directory
        if not tmp_path.is_file():
            raise HTTPException(status_code=500, 
                                detail=f"Expected file, but got directory at {tmp_path}.")

        print(f"Returning file: {tmp_path} (size: {tmp_path.stat().st_size} bytes)")

        download_filename = f"ras_xs_huc{identifier}.gpkg"

        return FileResponse(path=str(tmp_path),
                            filename=download_filename,
                            media_type="application/geopackage+sqlite3",
                            headers={"Data_Source": f"{xstype}_xs",
...
async def get_xs_subset_by_huc_reach_gpkg(identifier: str = Path(...,
